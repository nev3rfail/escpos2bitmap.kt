...
data class EscPosState(
    ...
    var imageSlices: MutableList<BufferedImage> = mutableListOf(), // Assuming Image is a type provided by your multiplatform library
    var qrSize: Byte = 0,
    var qrCorrLevel: Byte = 0,
    var qrBuff: MutableList<Byte> = mutableListOf(),
    var pos: Pair<Int, Int> = Pair(0, 0),
    var currImg: BufferedImage // Assuming Image is a type provided by your multiplatform library
)
...
class EscPosConverter {
    companion object {
        fun escpos2bitmap(escpos: ByteArray, fontSize: Int, imgWidth: Int): BufferedImage {
        ...
        fun escpos2bitmap(escpos: ByteArray, escPosConfig: EscPosConfig): BufferedImage {
            return escpos2bitmap(escpos, escPosConfig, EscPosState(currImg = createNewImage(escPosConfig.imgWidth, escPosConfig.fontScale.y).apply {
                this.fill(255.toByte()) // Fill with white color
            }
            ))
        }

        fun escpos2bitmap(escpos: ByteArray,  escPosConfig: EscPosConfig, escPosState: EscPosState): BufferedImage {
